...
@interface ALSinglePhotoView() <UIScrollViewDelegate>
...
@implementation ALSinglePhotoView

- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        _isFullWidthForLandScape = YES;
...
- (ALWaitingView *)waitingView
{
    if (!_waitingView) {
        _waitingView = [[ALWaitingView alloc] init];
        _waitingView.mode = kWaitingViewModeLoopDiagram;
    }
    return _waitingView;
}
...
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder
{
    ...
    self.waitingView.center = CGPointMake(kAPPWidth * 0.5, KAppHeight * 0.5);
...
- (void)reloadImage
{
    [self setImageWithURL:self.imageUrl placeholderImage:_placeHolderImage];
}
...
- (void)adjustFrame
{
    ...
    if (self.imageview.image) {
        ...
        if (self.isFullWidthForLandScape) {//图片宽度始终==屏幕宽度(新浪微博就是这种效果)
